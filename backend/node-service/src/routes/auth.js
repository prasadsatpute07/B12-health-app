...
router.post('/register', [
  ...
  try {
    ...
    if (existing) return sendError(res, 'Email already registered. Please log in to your existing account.', 409);
...
router.post('/login', [
  ...
  try {
    ...
    if (!user || !user.password_hash) return sendError(res, 'No account found with this email. Please sign up first.', 401);

    const valid = await bcrypt.compare(password, user.password_hash);
    if (!valid) return sendError(res, 'Incorrect password. Please try again or reset your password.', 401);
