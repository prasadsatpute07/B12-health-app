...
export default function ProfileScreen({ navigation }) {
  ...
  const { user, guestName, streak, dailyLogs, serverStreak } = state;

  // Use server streak as authoritative if available, fall back to local streak number
  const displayStreak    = serverStreak?.current_streak ?? streak;
  const displayLongest   = serverStreak?.longest_streak  ?? streak;
  const displayCheckins  = serverStreak?.total_checkins  ?? dailyLogs?.length ?? 0;
      ...
      >
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.statLine}>
              <Text style={styles.statLab}>Longest streak ⭐</Text>
              <Text style={styles.statVal}>{displayLongest}</Text>
            </View>
            <View style={[styles.statLine, { borderBottomWidth: 0 }]}>
              <Text style={styles.statLab}>Check-ins logged</Text>
              <Text style={styles.statVal}>{displayCheckins}</Text>
            </View>
          </View>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              Alert.alert(
                '📋 Check-in History',
                'Full check-in history with detailed logs and trends is coming soon!',
                [{ text: 'OK' }]
              )
            }
          >
            <View style={styles.historyRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.historyLabel}>View Full History</Text>
                <Text style={styles.historyHint}>All your daily logs and trends</Text>
              </View>
